...
;var LeaderLine = (function() { // eslint-disable-line no-extra-semi
  ...
  function bindWindow(props, newWindow) {
    // console.log("bindWindow----", props, newWindow);
  ...
  function updateViewBox(props) {
    ...
    ['x', 'y', 'width', 'height'].forEach(function(boxKey) {
      ...
      if ((value = curBBox[boxKey]) !== aplBBox[boxKey]) {
        ...
        // todo 矫正相对位置
        if(props.fixed && props.fixed.id) {
          styles[BBOX_PROP[boxKey]] = boxKey === 'x' || boxKey === 'y' ? props.fixed[boxKey] : value + "px"
        } else {
  ...
  ATTACHMENTS = {
    pointAnchor: {
      ...
      removeOption: function(attachProps, boundTarget) {
        ...
        traceLog.add('</ATTACHMENTS.pointAnchor.removeOption>'); // new_edge4viewBox.forEachDEBUG/]
      },
